...
LINENO = -1
reserved = {'NFA': 'NFA', 'DFA': 'DFA', 'PDA': 'PDA', 'TM': 'TM'}
...
tokens = ['EPS', 'BLANK', 'COLON', 'COMMA', 'SEMICOLON', 'ARROW', 'OR', 'ID'
...
def default_line_attr():
    ...
    """
    return {
        "FromState": [],  # Always singleton, per line of markdown
...
def union_line_attr_list_fld(line_attr_list, field, mc_type):
    ...
    """
    #-- Error checks.
    assert (mc_type in {'DFA', 'NFA', 'PDA',
                        'TM'}), "Error: Illegal machine type given: " + mc_type
    for line_attr in line_attr_list:
        ...
        assert (length_ok_input_items(Sigma)
                ), "Error: Sigma contains a string longer than 1"

        assert (length_ok_input_items(G_in)
                ), "Error: G_in contains a string longer than 1"

        if mc_type != 'PDA':
            assert (length_ok_input_items(G_out)
...
def extend_rsltdict(D, key, val, Extend=False):
    ...
    if Extend:
        ...
        assert (key not in D), ("Error: Duplicate map at key " + str(key) +
                                "; duplicates: " + str(D[key]) + " and " +
                                str(val))
...
def form_delta(line_attr_list, mc_type):
    ...
    for attr in line_attr_list:
        ...
        if mc_type == 'PDA':
            ...
            assert (mc_type
                    in {'NFA',
                        'DFA'}), "Error: Unknown machine type: " + mc_type
            ziplab = sigeps_list
        ...
        for zl_nxtst in l_zl_nxtst:
            if mc_type == 'DFA':
                ...
                rslt_dict = extend_rsltdict(rslt_dict, (from_state, inpsym),
                                            nxtst,
                                            Extend=False)
            elif mc_type == 'NFA':
                ...
                rslt_dict = extend_rsltdict(rslt_dict, (from_state, tapein),
                                            (nxtst, tapeout, dirn),
                                            Extend=True)
    #--
    return rslt_dict
...
def get_machine_components(line_attr_list, mc_type):
    ...
    #
    From_s = union_line_attr_list_fld(line_attr_list, "FromState", mc_type)
    To_s = union_line_attr_list_fld(line_attr_list, "ToStates", mc_type)
    #
    G_in = union_line_attr_list_fld(line_attr_list, "GammaIn", mc_type)
    G_out = union_line_attr_list_fld(line_attr_list, "GammaOut", mc_type)
    #
    Dirn = union_line_attr_list_fld(line_attr_list, "HeadDirn", mc_type)
    #
    Q0 = union_line_attr_list_fld(line_attr_list, "Q0", mc_type)
    #
    F = union_line_attr_list_fld(line_attr_list, "F", mc_type)
    #
    Sigma = union_line_attr_list_fld(line_attr_list, "SigmaEps", mc_type)

    Delta = form_delta(line_attr_list, mc_type)

    return (set(From_s), set(To_s), set(G_in), set(G_out), set(Q0), set(F),
            set(Sigma), set(Dirn), Delta)
...
def p_dfa_md(t):
    ...
    (From_s, To_s, G_in, G_out, Q0, F, Sigma, Dirn, Delta) = mc
    assert (len(Q0) == 1), "Error: DFA with " + str(
        len(Q0)) + " starting states is illegal."
    global LINENO
...
def p_tm_md(t):
    ...
    (From_s, To_s, G_in, G_out, Q0, F, Sigma, Dirn, Delta) = mc
    assert (len(Q0) == 1), "TM with more than one starting state illegal."
    global LINENO
    LINENO = -1  # restore for next error processing
    t[0] = ('TM', mc)
...
def find_initial_and_final_states(t, n):
    final_states = []
    initial_states = []
    ...
    for i in range(len(t[n])):
        ...
        # Case where state could be 'IF'
            ...
            # Cut off prefixes
            if state[0:2].upper() == 'IF':
                t[n][i] = t[n][i][2:]
                final_states.append(t[n][i])
                initial_states.append(t[n][i])
            elif state[0].upper() == 'F':
                t[n][i] = t[n][i][1:]
                final_states.append(t[n][i])
            elif state[0].upper() == 'I':
                t[n][i] = t[n][i][1:]
                initial_states.append(t[n][i])
...
def p_one_line(t):
    ...
    initial_1, final_1 = find_initial_and_final_states(t, 1)
    initial_5, final_5 = find_initial_and_final_states(t, 5)
    initial_states = list(set(initial_1 + initial_5))
    final_states = list(set(final_1 + final_5))
    ...
    # Initial states
    lineattr["Q0"] = initial_states

    # Final states
    lineattr["F"] = final_states
    #

...
def is_consistent_dfa(D):
    ...
    """
    return (is_partially_consistent_dfa(D)
            and set(fn_dom(D["Delta"])) == set(product(D["Q"], D["Sigma"])))
...
def is_partially_consistent_dfa(D):
    ...
    # -    assert(not('BH' in Q))
    return (Q != {} and Sigma != {} and fn_dom_Delta <= set(product(Q, Sigma))
            and fn_range_Delta != {} and fn_range_Delta <= Q and q0 in Q
            and F <= Q)
...
def is_consistent_nfa(N):
    ...
    return (Q != {} and Sigma != {} and "" not in Sigma
            and fn_dom_Delta <= set(product(Q, Sigma_w_Eps)) and fn_range_Check
            and Q0 <= Q and F <= Q)
...
def is_consistent_gnfa(G):
    ...
    return (Q != {} and fn_dom_Delta <= Q and fn_range_Check and Q0 <= Q
            and F <= Q)
...
def chk_consistent_pda(P):
    ...
    for trans in Delta_trans:
        ...
        symb = trSymb(trans)
        assert ((symb in Gamma)
                or (symb == "")), "Delta has illegal input symbol."
        stksymb = trStkSymb(trans)
        assert ((stksymb in Gamma)
                or (stksymb == "")), "Delta has illegal stack symbol."
        for trg in trTrg(trans):
            assert (pdaNstate(trg)
                    in Q), "PDA jumps to illegal state in its Delta."
            nstkstr = pdaNstkStr(trg)
            assert (type(nstkstr) == str
                    ), "PDA's 'new stack string' isn't a string."
            set_of_pushes = set(nstkstr)
            assert (set_of_pushes <=
                    Gamma), "PDA pushes illegal symbol into its stack."
...
def chk_consistent_tm(T):
    ...
    for trans in Delta_trans:
        ...
        for trg in trTrg(trans):
            assert (tmNstate(trg)
                    in Q), "TM jumps to illegal state in its Delta."
            assert (tmNtpSym(trg)
                    in Gamma), "TM writes illegal tape symbol in its Delta."
            assert (tmNdir(trg)
                    in {"L", "R",
                        "S"}), "TM has illegal direction in its Delta."
...
def prNodeDefs(D, pred):
    ...
    nonfinalstates = "".join(
        [prNonFinalNodeName(q) for q in (D["Q"] - D["F"]) if pred(q)])
    # Final state won't be black-hole!
    finalstates = "".join([prFinalNodeName(q) for q in D["F"]])
...
def prEdges(D, pred):
    ...
    listedges = [
        dot_san_str(trSrc(trans)) + ' -> ' + dot_san_str(trTrg(trans)) +
        ...
        if pred(trSrc(trans)) and pred(trTrg(trans)):
            '''
             dotObj.edge(dot_san_str(trSrc(trans)),
                         ...
                         label = dot_san_str(trSymb(trans)) )
             '''
            dotAl += [((dot_san_str(trSrc(trans)), dot_san_str(trTrg(trans))),
...
def prNFANodeDefs(N):
    ...
    emptystate = '"" [shape=plaintext];\n'
    nonfinalstates = "".join(
        [prNonFinalNodeName(q) for q in (N["Q"] - N["F"])])
    finalstates = "".join([prFinalNodeName(q) for q in N["F"]])
    return nodescmt + emptystate + nonfinalstates + finalstates
...
def addNFAEdges(N, dotObj, FuseEdges, visible_eps=False):
    ...
    for trans in fn_trans(N["Delta"]):
        for nxt_state in trTrg(trans):
            dotAl += [((dot_san_str(trSrc(trans)), dot_san_str(nxt_state)),
...
def fuseDotAl(dotAl):
    ...
    """
    d = dict([(x, {y
                   for (z, y) in dotAl if z == x})
              for (x, w) in dotAl])  # sets of y for the same x
    return [(a, reduce(lambda x, y: x + " \n " + y, b))
            for (a, b) in d.items()]  # concat the ys now
...
def addPDAEdges(P, dotObj, FuseEdges, visible_eps):
    ...
    for trans in fn_trans(P["Delta"]):
        ...
        for trg in trTrg(trans):
            ...
            nstkstr = pdaNstkStr(trg)
            dotAl += [((dot_san_str(trSrc(trans)), dot_san_str(nstate)),
                       pdaEdgLab(tr_isymb, tr_ssymb, nstkstr, visible_eps))]
...
def tmEdgLab(tr_isymb, ntpsym, direction):
    return (tr_isymb + " ; " + ntpsym + "," + direction)
